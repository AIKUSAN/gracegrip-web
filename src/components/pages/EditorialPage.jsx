...
export function EditorialPage() {
  ...
  return <div className="v2-page v2-editor">
    <header className="v2-intro"><h1>Private article editor</h1><p>Drafts stay in editorial D1. Owner approval prepares a draft publishing pull request; merging protected main publishes an article.</p></header>
    {error && <p role="alert" className="v2-account-error">{error}</p>}
    {notice && <p role="status" className="v2-account-notice">{notice}</p>}
    <div className="v2-editor-layout"><aside><h2>Drafts</h2><button type="button" onClick={() => { setId(null); setRevision(null); setStatus('draft'); setArticle(blank); setReviewed(false) }}>New private draft</button><ul>{drafts.map((draft) => <li key={draft.id}><button type="button" onClick={() => openDraft(draft.id)}>{draft.title} <small>({draft.status})</small></button></li>)}</ul></aside>
      <div><section><h2>{id ? 'Edit draft' : 'New draft'}</h2><p>State: {status}{id ? ` · revision ${revision}` : ''}{reviewed ? ' · qualified review current' : ''}</p>
        ...
        <fieldset><legend>Sections</legend>{article.sections.map((section, index) => <div className="v2-editor-section" key={index}><label>Heading {index + 1}<input value={section.heading} maxLength={120} onChange={(event) => updateSection(index, 'heading', event.target.value)} disabled={busy || !['draft', 'changes_requested'].includes(status)} /></label><label>Body {index + 1}<textarea rows={8} value={section.body} maxLength={3000} onChange={(event) => updateSection(index, 'body', event.target.value)} disabled={busy || !['draft', 'changes_requested'].includes(status)} /></label><button type="button" disabled={busy || article.sections.length === 1} onClick={() => update('sections', article.sections.filter((_, position) => position !== index))}>Remove section</button></div>)}</fieldset>
