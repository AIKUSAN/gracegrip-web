...
export function AccountPage() {
  ...
  return <div className="cc-page cc-account">
    <header className="cc-intro"><h1>Private membership</h1><p>Public help does not need an account. Membership is optional for selected progress sync and, after moderation review, community participation. Your journal stays on this device.</p></header>
    {error && <p className="cc-account-error" role="alert">{error}</p>}
    {notice && <p className="cc-account-notice" role="status">{notice}</p>}
    {session === null && !error && <p role="status">Checking membership availability…</p>}
    {session && !session.signedIn && <div className="cc-account-grid">
      <section><h2>New member</h2><p>Create a passkey. Your browser or password manager may offer a synced passkey; availability depends on your device and settings.</p><label className="cc-check-label"><input type="checkbox" checked={adultConfirmed} onChange={(event) => setAdultConfirmed(event.target.checked)} /> I am 18 or older.</label><button className="cc-action" type="button" disabled={busy || !adultConfirmed} onClick={createPasskey}>Create with a passkey</button></section>
      <section><h2>Already a member</h2><p>Sign in with a passkey to review your synced copy. No device data moves automatically.</p><button className="cc-action" type="button" disabled={busy} onClick={signIn}>Sign in</button></section>
    ...
    {session?.signedIn && <div className="cc-account-grid">
      {recoveryCodes.length > 0 && <section className="cc-recovery-codes"><h2>Save these recovery codes</h2><p>Shown once. Each code works once. Keep them in a safe place separate from this device.</p><ul>{recoveryCodes.map((code) => <li key={code}><code>{code}</code></li>)}</ul><button type="button" onClick={() => setRecoveryCodes([])}>I have saved them</button></section>}
      ...
      <section><h2>Opt-in progress sync</h2><p>Select only goals you want to copy to your account. Your journal, legacy streak, profile name, puzzle score, and unselected goals are never included.</p>
        {(appState.goals ?? []).length ? (appState.goals ?? []).map((goal) => {
          ...
          return <label key={goal.focusId} className="cc-check-label"><input type="checkbox" checked={selected.includes(goal.focusId)} onChange={(event) => setSelected((current) => event.target.checked ? [...current, goal.focusId] : current.filter((id) => id !== goal.focusId))} /> {area?.title ?? goal.focusId}</label>
        }) : <p>No device-local goals yet. Choose a focus before syncing.</p>}
        <label className="cc-check-label"><input type="checkbox" checked={confirmedSync} onChange={(event) => setConfirmedSync(event.target.checked)} /> I understand this replaces my previous selected cloud copy.</label>
        <button type="button" disabled={busy || !confirmedSync || selected.length === 0} onClick={upload}>Upload only selected progress</button>
      </section>
      <section><h2>Cloud copy</h2><p>Review the cloud copy before choosing whether to bring it to this device.</p><button type="button" disabled={busy} onClick={() => run(loadRemote)}>Review cloud copy</button>{remote && <div className="cc-cloud-summary"><p>Revision {remote.revision} · {remote.syncedAt ? `Last updated ${new Date(remote.syncedAt).toLocaleDateString()}` : 'No upload yet'}</p><p>{remote.progress?.goals?.length ?? 0} synced goals · {remote.progress?.goalCheckins?.length ?? 0} check-ins</p>{remote.progress && <button type="button" disabled={busy} onClick={importRemote}>Copy selected cloud progress to this device</button>}</div>}</section>
