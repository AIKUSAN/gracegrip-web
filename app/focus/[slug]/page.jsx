...
export default async function FocusGuide({ params }) {
  ...
  const area = getFocusArea(slug)
  if (!area) notFound()
  return <AppShell allowBeforeOnboarding><article className="cc-page cc-guide">
    <Link className="cc-back" href="/focus">← All focus areas</Link>
    <header className="cc-intro"><h1>{area.title}</h1><p>{area.summary}</p></header>
    <div className="cc-guide-columns"><div>
      <section><h2>One next step</h2><p>{area.firstStep}</p><Link className="cc-action" href="/emergency">Open Help Now</Link></section>
    ...
    </div><aside><section className="cc-guide-caution"><h2>When to seek help</h2><p>{area.caution}</p><p>{area.professional}</p><p>If there is immediate danger, contact local emergency services. For country-searchable emotional support, use the <a href="https://www.iasp.info/crisis-centres-helplines/" target="_blank" rel="noopener noreferrer">IASP helpline directory</a>.</p></section><GoalChoice area={area} /></aside></div>
    <p className="cc-disclosure">GraceGrip offers nonclinical self-help. It does not diagnose, treat withdrawal, prescribe medication, or replace a qualified professional. <a href={area.source} target="_blank" rel="noopener noreferrer">Source for further reading</a>.</p>
  </article></AppShell>
}
